...
@Service
public class GlmChatCompletionsServiceImpl implements GlmChatCompletionsService {
    ...
    public GlmChatCompletionsServiceImpl(
            CrudMapper crudMapper,
            RestTemplate restTemplate,
            @Value("${ai-chat.glm.chat-completions-url:https://open.bigmodel.cn/api/paas/v4/chat/completions}") String chatCompletionsUrl,
    ...
    @Override
    public String createChatCompletion(String requestBody, HttpHeaders requestHeaders) {
        String responseBody = callGlmChatCompletions(requestBody, requestHeaders);
        saveResponse(responseBody);
        return responseBody;
    }

    private String callGlmChatCompletions(String requestBody, HttpHeaders requestHeaders) {
        HttpHeaders headers = buildRemoteHeaders(requestHeaders);
        HttpEntity<String> request = new HttpEntity<>(requestBody, headers);
        ResponseEntity<String> response = restTemplate.exchange(chatCompletionsUrl, HttpMethod.POST, request, String.class);
    ...
    private void saveResponse(String responseBody) {
        ...
        record.setUrl(chatCompletionsUrl);
